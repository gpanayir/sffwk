...
using System.IO;
using System.Text;
using Fwk.Controls.Win32.DocpanelEnums;
...
namespace Fwk.Controls.Win32 {
  ...
  public class DockPanel : Panel {
      private const int WM_REFRESHACTIVEWINDOW = (int)Fwk.Controls.Win32.DocpanelEnums.Msgs.WM_USER + 1;
    ...
    private MdiClientController m_mdiClienteController = null;
    private MdiClientController MdiClientController {
      get {
        if (m_mdiClienteController == null) {
          m_mdiClienteController = new MdiClientController();
          m_mdiClienteController.HandleAssigned += new EventHandler(MdiClientHandleAssigned);
          m_mdiClienteController.MdiChildActivate += new EventHandler(ParentFormMdiChildActivate);
          m_mdiClienteController.Layout += new LayoutEventHandler(MdiClient_Layout);
        }

        return m_mdiClienteController;
      }
    }
      ...
      lock (this) {
        if (!m_disposed && disposing) {
          ...
          if (m_mdiClienteController != null) {
            ...
            m_mdiClienteController.MdiChildActivate -= new EventHandler(ParentFormMdiChildActivate);
            m_mdiClienteController.Layout -= new LayoutEventHandler(MdiClient_Layout);
            m_mdiClienteController.Dispose();
          }
    ...
    protected override void WndProc(ref Message m) {
      if (m.Msg == WM_REFRESHACTIVEWINDOW)
        RefreshActiveWindow();
      else if (m.Msg == (int)Msgs.WM_WINDOWPOSCHANGING) {
        int offset = (int)Marshal.OffsetOf(typeof(Win32.WINDOWPOS), "flags");
        int flags = Marshal.ReadInt32(m.LParam, offset);
        Marshal.WriteInt32(m.LParam, offset, flags | (int)FlagsSetWindowPos.SWP_NOCOPYBITS);
      }
