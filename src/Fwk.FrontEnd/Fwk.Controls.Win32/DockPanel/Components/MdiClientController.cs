...
namespace Fwk.Controls.Win32
{
	...
	internal class MdiClientController : NativeWindow, IComponent, IDisposable
	{
		...
		private MdiClient m_mdiCliente = null;
		...
		public BorderStyle BorderStyle
		{
			...
			set
			{
				...
				int style = User32.GetWindowLong(MdiClient.Handle, (int)GetWindowLongIndex.GWL_STYLE);
				int exStyle = User32.GetWindowLong(MdiClient.Handle, (int)GetWindowLongIndex.GWL_EXSTYLE);
				...
				User32.SetWindowLong(MdiClient.Handle, (int)GetWindowLongIndex.GWL_STYLE, style);
				User32.SetWindowLong(MdiClient.Handle, (int)GetWindowLongIndex.GWL_EXSTYLE, exStyle);
		...
		private void MdiClientHandleDestroyed(object sender, EventArgs e)
		{
			...
			// release the handle.
			if(m_mdiCliente != null)
			{
				m_mdiCliente.HandleDestroyed -= new EventHandler(MdiClientHandleDestroyed);
				m_mdiCliente = null;
			}
		...
		{
			// If the mdiCliente has previously been set, unwire events connected
			// to the old MDI.
			if(MdiClient != null)
			{
			...
			foreach (Control control in ParentForm.Controls)
			{
				...
				m_mdiCliente = control as MdiClient;
				if(m_mdiCliente == null)
					continue;
