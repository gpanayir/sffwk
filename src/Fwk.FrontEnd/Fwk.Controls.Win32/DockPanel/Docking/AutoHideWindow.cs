...
namespace Fwk.Controls.Win32
{
	internal class AutoHideWindow : Panel
	{
		...
		private void LayoutAnimateWindow(Rectangle rect)
		{
			...
			Rectangle rectCliente = ClientRectangle;

			if (DockState == DockState.DockLeftAutoHide)
				ActivePane.Location = new Point(rectCliente.Right - 2 - Measures.SplitterSize - ActivePane.Width, ActivePane.Location.Y);
			else if (DockState == DockState.DockTopAutoHide)
				ActivePane.Location = new Point(ActivePane.Location.X, rectCliente.Bottom - 2 - Measures.SplitterSize - ActivePane.Height);
		}
		...
		private void SetTimerMouseTrack()
		{
			...
			User32.SystemParametersInfo(SystemParametersInfoActions.GetMouseHoverTime, 0, ref hovertime, 0);
