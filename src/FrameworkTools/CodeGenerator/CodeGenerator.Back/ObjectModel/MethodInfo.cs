...
namespace CodeGenerator.Back.ObjectModel
{
	...
	public class MethodInfo
	{
		...
		/// <summary>
		/// Indica si el método es Built-in o no.
		/// </summary>
	    /// <author>Marcelo Oviedo</author>
		public bool BuiltIn
		{
			get
			{
				return (0 != (int)_Action && _Action != MethodActionType.Get && _Action != MethodActionType.Other);
			}
		}
        ...
        private static bool CheckMappingNeed(fwkDataEntities.Column pTableFieldInfo, MethodActionType pMethodActionType)
        {
            ...
            bool wResult = !(Convert.ToBoolean(pMethodActionType & (MethodActionType.GetAll )));

            //                     Si KeyField  o  (No KeyField Y (Insert o Update o Get) = True
            wResult = wResult && (pTableFieldInfo.KeyField || (!pTableFieldInfo.KeyField && Convert.ToBoolean(pMethodActionType & (MethodActionType.Insert | MethodActionType.Update | MethodActionType.Get | MethodActionType.GetByParam))));
            return wResult;
        }
        ...
        {   //Si es GetAllPaginated o  GetAll = False
            ...
            wResult = wResult && (Convert.ToBoolean(pMethodActionType & (MethodActionType.Insert | MethodActionType.Update | MethodActionType.Get | MethodActionType.GetByParam)));
            return wResult;
        }
        ...
        public static void GenerateParameters(MethodInfo pMethodInfo)
        {
            ...
            if (pMethodInfo.Entity.Table != null && pMethodInfo.BuiltIn)
                {


                    foreach (fwkDataEntities.Column wColumn in pMethodInfo.Entity.Table.Columns)
                    {
                        if (CheckMappingNeed(wColumn,pMethodInfo.Action))
                        {
                            ...
                            try
                            {
                                ...
                                wParameterInfo.Direction = ((pMethodInfo.Action == MethodActionType.Insert) && wColumn.Autogenerated) ? ParameterDirection.Output : ParameterDirection.Input;
                ...
                if (pMethodInfo.Entity.StoredProcedure != null && pMethodInfo.BuiltIn)
                {
                    ...
                    {
                        if (CheckMappingNeed(wSPParameter,pMethodInfo.Action))
                        {
                            ParameterInfo wParameterInfo = null;
                            ...
                            try
                            {
                                ...
                                wParameterInfo.Direction = (pMethodInfo.Action == MethodActionType.Insert) ? ParameterDirection.Output : ParameterDirection.Input;
                                pMethodInfo.Parameters.Add(wParameterInfo);
                            }
                            finally
                            {
