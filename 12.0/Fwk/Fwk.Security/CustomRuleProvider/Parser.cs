...
namespace Fwk.Security
{
    public class Parser
    {
        ...
        private void AssertTokenType(TokenType expected)
        {
            ...
            {
                string message = string.Format(CultureInfo.CurrentCulture, Fwk.Security.Properties.Resource.UnexpectedTokenMessage,
                    this.GetTokenName(this.token),
                    this.GetTokenName(expected),
                    this.GetIndex());
                throw new Fwk.Exceptions.TechnicalException (message);
            }
            this.MoveNext();
        }
